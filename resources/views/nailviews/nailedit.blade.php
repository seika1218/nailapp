...
<body>
    <main class="">
        <form method="post" action="/naileditcomplete/{{$nailedit->id}}" name="form">
            @csrf
            <img class="naildash1 imgedit listimg" src="{{ asset($nailedit->img_path) }}">
            <br>
            <textarea rows="10" cols="60" name="explanation" class="con-btn-large textedit">{{ $nailedit->explanation }}</textarea>
            <br>
            <input type="submit" class="con-enter btnedit" name="button" id="per" value="更新">
        </form>

        <div class="forget-password back"><a href="{{ url()->previous() }}">戻る</a></div>
    </main>
    <footer>
